...
RSpec.describe "catalog/_document" do
  ...
  let(:cache) { Blacklight::ViewCache.new }
  let(:renderer) { instance_double(Blacklight::IndexDocumentPartialsRenderer, render: '') }
  ...
  it "renders the header, thumbnail and index by default" do
    ...
    render partial: "catalog/document", locals: { document: document, document_counter: 1, view_cache: cache }
  ...
  it 'has a css class with the document position' do
    allow(Blacklight::IndexDocumentPartialsRenderer).to receive(:new).and_return(renderer)
    render partial: 'catalog/document', locals: { document: document, document_counter: 5, view_cache: cache }
