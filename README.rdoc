...
To run the tests:
  ...
  * From the blacklight source checkout directory, you'll need to install
    a test Solr and load it with the expected test data. A test jetty
    install and test data are linked to Blacklight source with git
    submodule. 
    * git submodule init
  ...
  * Now use some rake tasks that come with Blacklight to actually run the tests. 
    ...
    * rake blacklight:rspec:with_solr
    * rake blacklight:rspec
