...
module Blacklight
  class GuidedSearchFormComponent < SearchBarComponent
    ...
    renders_many :search_filter_controls, (lambda do |config:, display_facet:, presenter: nil, component: nil, **kwargs|
      ...
      component ||= config.guided_search_component
    ...
    def default_operator_menu
      options_with_labels = [:must, :should].index_by { |op| t(op, scope: 'blacklight.guided_search.op') }
      label_tag(:op, t('blacklight.guided_search.op.label'), class: 'visually-hidden') + select_tag(:op, options_for_select(options_with_labels, params[:op]), class: 'input-small')
    end
    ...
    def sort_fields_select
      ...
      select_tag(:sort, options_for_select(options, params[:sort]), class: "form-select custom-select sort-select w-auto", aria: { labelledby: 'guided-search-sort-label' })
    end
      ...
      search_fields.values.each.with_index do |field, i|
        with_search_field_control do
          fields_for('clause[]', i, include_id: false) do |f|
            content_tag(:div, class: 'form-group guided-search-field row mb-3') do
    ...
    def initialize_search_filter_controls
      fields = blacklight_config.facet_fields.select { |_k, v| v.include_in_guided_search || v.include_in_guided_search.nil? }
    ...
    def initialize_constraints
      ...
      guided_search_context = helpers.search_state.reset(params)

      constraints_text = render(Blacklight::ConstraintsComponent.for_search_history(search_state: guided_search_context))
    ...
    def search_fields
      blacklight_config.search_fields.select { |_k, v| v.include_in_guided_search || v.include_in_guided_search.nil? }
    end

    def sort_fields
      blacklight_config.sort_fields.select { |_k, v| v.include_in_guided_search || v.include_in_guided_search.nil? }
    end
  end
end
